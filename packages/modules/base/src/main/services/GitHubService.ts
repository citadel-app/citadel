...
export class GitHubService {
  ...
  private registerIpcHandlers() {
    this.registrar.handle('github.createRepository', async (token: string, name: string, description: string, isPrivate: boolean) => {
      return this.createRepository(token, name, description, isPrivate);
    });
    this.registrar.handle('github.listRepos', async (token: string) => {
      return this.listRepos(token);
    });
    this.registrar.handle('github.forkRepository', async (token: string, owner: string, repo: string) => {

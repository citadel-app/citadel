...
import { VectorStoreProvider, VectorPoint } from '@citadel-app/core';
...
export class QdrantStore implements VectorStoreProvider {
    ...
    async checkConnection(): Promise<boolean> {
        try {
            const response = await net.fetch(`${this.baseUrl}/collections`);
            return response.ok;
        } catch {
            return false;
        }
    ...
    async ensureCollection(vectorSize: number = 768): Promise<boolean> {
        try {
            const checkResp = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}`);
            if (checkResp.ok) return true;

            const createResp = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}`, {
                method: 'PUT',
            ...
            if (!createResp.ok) {
                ...
                console.error('[QdrantStore] Failed to create collection:', errorText);
                return false;
            }

            // Create payload index for entryId
            await net.fetch(`${this.baseUrl}/collections/${this.collectionName}/index`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    field_name: 'entryId',
    ...
    async upsert(points: VectorPoint[]): Promise<boolean> {
        try {
            const response = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}/points`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    async search(vector: number[], limit: number = 10, filter?: any): Promise<any[]> {
        try {
            ...
            const response = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}/points/search`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async deleteByFilter(filter: any): Promise<boolean> {
        try {
            const response = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}/points/delete`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async clearAll(): Promise<boolean> {
        try {
            await net.fetch(`${this.baseUrl}/collections/${this.collectionName}`, { method: 'DELETE' });
    ...
    async getPointsByFilter(filter: any, limit: number = 10): Promise<any[]> {
        try {
            ...
            const response = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}/points/scroll`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(body)
            });
    ...
    async deleteCollection(): Promise<boolean> {
        try {
            const response = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}`, {
                method: 'DELETE'
            });
            return response.ok;
        } catch (e) {
    ...
    async getEntryVectorCount(entryId: string): Promise<number> {
        try {
            ...
            const response = await net.fetch(`${this.baseUrl}/collections/${this.collectionName}/points/count`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(body)
            });
    ...
    async getCollectionInfo(name: string): Promise<any> {
        try {
            const response = await net.fetch(`${this.baseUrl}/collections/${name}`);
            if (!response.ok) return null;
            const data = await response.json() as any;

...
import { PRESETS, type WorkspacePreset } from '@citadel-app/core';
import { DEFAULT_WORKSPACE_CONFIG, type WorkspaceConfig, type EntryTypeConfig, type ModuleDefinition } from '@citadel-app/core';
...
export const WorkspaceBuilderPage: FC = () => {
    ...
    const [draftConfig, setDraftConfig] = useState<WorkspaceConfig>(() => {
        const config = JSON.parse(JSON.stringify(DEFAULT_WORKSPACE_CONFIG));
        return config;
    });
    ...
    useEffect(() => {
        const loadGitHub = async () => {
            try {
                const raw = await __hostApi.module.invoke('@citadel-app/base', 'secrets.get', 'github_accounts');
                if (raw) {
    ...
    const handleCreateWorkspace = async () => {
        ...
        try {
            ...
            if (pushToGithub && githubAccount && repoName) {
                ...
                const repoData = await __hostApi.module.invoke('@citadel-app/base', 'github.createRepository', 
                ...
                await __hostApi.module.invoke('@citadel-app/base', 'git.setConfig', localPath, 'user.name', githubAccount.name || githubAccount.login);
                await __hostApi.module.invoke('@citadel-app/base', 'git.setConfig', localPath, 'user.email', `${githubAccount.login}@users.noreply.github.com`);
    ...
    const moduleCount = appModuleRegistry.getContentModules().length;
                        ...
                        {editorMode === 'ui' ? (
                            <div className="space-y-8">
                                ...
                                <div className="pt-4 border-t border-border mt-4">
                                    <ModuleList
                                        modules={appModuleRegistry.getContentModules()}
                                    />
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-3">
                                ...
                                <div className="border border-border rounded-xl overflow-hidden">
                                    <textarea
                                        className="w-full h-[500px] bg-background text-foreground font-mono text-xs p-4 outline-none resize-none"
                                        value={json}
                        ...
                        <div className="p-5 rounded-2xl bg-card/60 border border-muted space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="flex flex-wrap gap-1.5 pt-1">
                                        {appModuleRegistry.getContentModules().slice(0, 4).map(m => (

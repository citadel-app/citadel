...
import { DynamicIcon } from '@citadel-app/ui';
import { DEFAULT_WORKSPACE_CONFIG, type WorkspaceConfig, type EntryTypeConfig, type WorkspacePreset, PRESETS, APP_CONSTANTS } from '@citadel-app/core';
...
export const WelcomePage: React.FC = () => {
    ...
    const saveAccounts = async (accs: GitHubAccount[], active: string | null) => {
        await __hostApi.module.invoke('@citadel-app/base', 'secrets.set', 'github_accounts', JSON.stringify(accs));
        if (active) await __hostApi.module.invoke('@citadel-app/base', 'secrets.set', 'github_active_login', active);
    };
    ...
    const switchAccount = async (login: string) => {
        ...
        await __hostApi.module.invoke('@citadel-app/base', 'secrets.set', 'github_active_login', login);
    ...
    const logout = async (login: string) => {
        ...
        if (activeLogin === login) {
            ...
            if (newActive) {
                ...
                await __hostApi.module.invoke('@citadel-app/base', 'secrets.set', 'github_accounts', JSON.stringify(updated));
                await __hostApi.module.invoke('@citadel-app/base', 'secrets.set', 'github_active_login', '');
                setStep('auth-gate');
            }
        } else {
    ...
    React.useEffect(() => {
        const loadExisting = async () => {
            ...
            const oldToken = await __hostApi.module.invoke('@citadel-app/base', 'secrets.get', 'github_token');
            ...
            if (accountsJson) {
                try {
                    ...
                    for (const acc of parsed) {
                        try {
                            await __hostApi.module.invoke('@citadel-app/base', 'github.getUser', acc.token);
                            valid.push(acc);
                        } catch {
                            // Token expired, skip
                        }
                    ...
                    if (valid.length > 0) {
                        setAccounts(valid);
                        const activePref = await __hostApi.module.invoke('@citadel-app/base', 'secrets.get', 'github_active_login');
    ...
    const startDeviceFlow = async () => {
        try {
            ...
            const flow = await __hostApi.module.invoke('@citadel-app/base', 'github.startDeviceFlow');
            setUserCode(flow.user_code);
            setVerificationUri(flow.verification_uri);
            __hostApi.app.openExternal(flow.verification_uri);

            let interval = (flow.interval || 5) * 1000;
            const pollFn = async () => {
                try {
                    const result = await __hostApi.module.invoke('@citadel-app/base', 'github.pollDeviceToken', flow.device_code);
    ...
    const validateWorkspace = async (path: string): Promise<boolean> => {
        // Explicitly authorize the path in the guardrail service before querying the file system
        await __hostApi.module.invoke('@citadel-app/base', 'fs.allowPath', path);
        const configDir = `${path}/${APP_CONSTANTS.PATHS.CONFIG_DIR}`;
        const isValid = await __hostApi.module.invoke('@citadel-app/base', 'fs.exists', configDir);
        if (!isValid) {
    ...
    const handleOpenFolder = async () => {
        const path = await __hostApi.dialog.openDirectory();
        if (path) {
    ...
    const loadRepos = async () => {
        ...
        try {
            const data = await __hostApi.module.invoke('@citadel-app/base', 'github.listRepos', githubToken);
            setRepos(data);
        } catch (e: any) {
    ...
    const handleCloneRepo = async () => {
        ...
        try {
            ...
            await __hostApi.module.invoke('@citadel-app/base', 'git.clone', authUrl, clonePath);
    ...
    const handleCloneRemoteRepo = async () => {
        ...
        try {
            ...
            if (cloneMethod === 'fork') {
                ...
                const forkedRepo = await __hostApi.module.invoke('@citadel-app/base', 'github.forkRepository', activeCloneToken, owner, repo);
                cloneUrl = forkedRepo.clone_url;

                // Auth URL
                const authUrl = cloneUrl.replace('https://', `https://${activeCloneToken}@`);
                await __hostApi.module.invoke('@citadel-app/base', 'git.clone', authUrl, cloneRemotePath);
                ...
                if (activeCloneToken) {
                    ...
                    await __hostApi.module.invoke('@citadel-app/base', 'git.clone', cloneUrl, cloneRemotePath);
                }
            } else if (cloneMethod === 'disconnected') {
                // Clone anonymously
                await __hostApi.module.invoke('@citadel-app/base', 'git.clone', cloneUrl, cloneRemotePath);
                // Remove remote origin, treating it as completely disconnected and local
                await __hostApi.module.invoke('@citadel-app/base', 'git.removeRemote', cloneRemotePath, 'origin');
            }
    ...
    const handleCreateRemoteRepo = async () => {
        ...
        setCreateError(null);
        try {
            const repoData = await __hostApi.module.invoke('@citadel-app/base', 'github.createRepository', githubToken, repoName, repoDesc, isPrivate);
            ...
            await __hostApi.module.invoke('@citadel-app/base', 'fs.allowPath', localPath);

            // Generate templated files (README, etc.) *before* Codex config
            await __hostApi.module.invoke('@citadel-app/base', 'fs.scaffoldWorkspace', localPath, repoData.full_name, repoData.clone_url);

            // Bootstrap Codex configuration on top of the template
            await dataManager.createWorkspace(localPath, config);

            await __hostApi.module.invoke('@citadel-app/base', 'git.init', localPath);
            // Set workspace-specific git user from GitHub account
            if (githubUser) {
                await __hostApi.module.invoke('@citadel-app/base', 'git.setConfig', localPath, 'user.name', githubUser.name || githubUser.login);
                await __hostApi.module.invoke('@citadel-app/base', 'git.setConfig', localPath, 'user.email', `${githubUser.login}@users.noreply.github.com`);
            }
            const authUrl = repoData.clone_url.replace('https://', `https://${githubToken}@`);
            await __hostApi.module.invoke('@citadel-app/base', 'git.addRemote', localPath, 'origin', authUrl);
            await __hostApi.module.invoke('@citadel-app/base', 'git.add', localPath, ['.']);
            await __hostApi.module.invoke('@citadel-app/base', 'git.commit', localPath, 'Initial commit from Citadel');
            await __hostApi.module.invoke('@citadel-app/base', 'git.push', localPath, 'origin', 'main');
            localStorage.setItem('codex-show-tour', 'true');
    ...
    return (
        ...
        >
            ...
            <button
                onClick={() => __hostApi.window.close()}
                            ...
                            <div className="flex flex-col gap-6 w-full items-center">
                                ...
                                {isPolling && userCode && (
                                    <div className="w-full max-w-md p-6 rounded-2xl bg-card/60 border border-primary/30 text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                        ...
                                        <button
                                            onClick={() => __hostApi.app.openExternal(verificationUri)}

...
export const EntryHeader = ({
    ...
    const handleAddLink = async (link: { id: string; type: string; title: string, url?: string }, extra?: any) => {
        ...
        // 2. Fire External Cross-Link Handlers
        const extensionHandlers = appModuleRegistry.getCrossLinkHandlers();
        for (const handler of extensionHandlers) {
            try {
                await handler.handleLinkCompleted(
                    [{ id: entry.id, type: entry.type, title: entry.title }], 
                    link, 
                    extra
                );
            } catch (e) {
                console.error(`CrossLinkHandler ${handler.id} failed`, e);
            }
        }

        // 3. Codex Internal Bidirectional Update
        // We attempt this for any link that might be in our database
        try {
    ...
    return (
        <div className="mb-8 pb-6 border-border">
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div
                                                        ...
                                                        onClick={() => {
                                                            if (isRss) {
                                                                ...
                                                                __hostApi.app.openExternal(link.url);
                                                            }
                                                        }}
                                                    >
                                                        <span
                                                    ...
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            setConfirmRemoveLinkId(link.id);
                                                        }}
            ...
            <ConfirmDialog
                open={!!confirmRemoveLinkId}
                onOpenChange={(open) => !open && setConfirmRemoveLinkId(null)}
                title="Remove Link"
                description="Are you sure you want to remove this link?"
                confirmLabel="Remove"
                onConfirm={async () => {
                    if (confirmRemoveLinkId) {
                        await dataManager.removeRelatedLink(entry.id, confirmRemoveLinkId);
                    }

...
export const BranchManagerModal = ({ onClose }: BranchManagerModalProps) => {
    ...
    const loadBranches = async () => {
        ...
        try {
            setLoading(true);
            const result = await __hostApi.module.invoke('@citadel-app/base', 'git.getBranches', vaultPath);
            setBranches(result);
        } catch (err) {
            console.error(err);
    ...
    const handleCheckout = async (branch: string) => {
        ...
        try {
            setLoading(true);
            await __hostApi.module.invoke('@citadel-app/base', 'git.checkout', vaultPath, branch);
            await refreshStatus();
            await loadBranches();
    ...
    const handleCreateBranch = async () => {
        ...
        try {
            setLoading(true);
            await __hostApi.module.invoke('@citadel-app/base', 'git.createBranch', vaultPath, newBranchName);
            await refreshStatus();
            await loadBranches();
            setNewBranchName('');
        } catch (e: any) {
    ...
    const executeDeleteBranch = async () => {
        if (!confirmDeleteBranch || !vaultPath) return;
        try {
            setLoading(true);
            await __hostApi.module.invoke('@citadel-app/base', 'git.deleteBranch', vaultPath, confirmDeleteBranch);
            await loadBranches();
        } catch (e) {
            console.error(e);
    ...
    return (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            ...
            <ConfirmDialog
                ...
                onOpenChange={(open) => !open && setConfirmDeleteBranch(null)}
                title="Delete Branch"
                description={`Delete branch '${confirmDeleteBranch}'? This cannot be undone.`}
                confirmLabel="Delete"

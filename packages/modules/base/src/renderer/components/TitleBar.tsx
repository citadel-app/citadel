...
import { Popover, PopoverTrigger, PopoverContent, PopoverAnchor, useAudio, useCommands } from '@citadel-app/ui';
...
export const TitleBar = () => {
    ...
    useEffect(() => {
        ...
        const unlisten = __hostApi.on('window:state-changed', (state: { isMaximized: boolean; isFullScreen: boolean }) => {
    ...
    const handleMinimize = () => __hostApi.window.minimize();
    const handleMaximize = () => __hostApi.window.maximize();
    const handleClose = () => __hostApi.window.close();
            ...
            <div className="flex items-center shrink-0 h-full ml-auto">
                ...
                <nav className="flex items-center gap-0.5 px-2" style={{ WebkitAppRegion: 'no-drag' } as any}>
                    ...
                    <div className="hidden sm:flex items-center gap-0.5">
                        {appModuleRegistry.getNavigationItems().map(item => (
                            <button
                                key={item.id}
                                ...
                                className={cn(
                                    ...
                                    location.pathname.startsWith(item.path) 
                                        ? (item.activeClass || "text-primary bg-primary/10") 
                                        : (item.inactiveClass || "text-muted-foreground hover:bg-muted")
                                )}
                                title={item.label}
                            >

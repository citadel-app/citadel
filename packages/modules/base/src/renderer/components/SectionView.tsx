...
import { MarkdownViewer } from '@citadel-app/ui';
import { useConfig } from '../context/ConfigContext';
import { type EntrySection, resolveResourceUrl } from '@citadel-app/core';
                ...
                <div className={cn(
                    ...
                    {(() => {
                        ...
                        if (editorType && editorType !== 'markdown' && editorType !== 'list') {
                            ...
                            if (DynamicEditor) {
                                return <DynamicEditor content={isEditing ? editedContent : section.content} onChange={setEditedContent} editable={isEditing} />;
                            }
                            // Fallback if plugin missing
                            return (
                                <div className="flex flex-col border rounded-md shadow-sm overflow-hidden min-h-[200px] items-center justify-center text-muted-foreground p-8 text-center bg-muted/5">
                                    <div>
                                        <p className="font-semibold mb-2 text-foreground">Missing Editor Module: <span className="font-mono text-primary">{editorType}</span></p>
                                        <p className="text-sm">The plugin required to read this section is missing.</p>
                                    </div>
                                    <div className="w-full text-left max-h-[300px] overflow-auto mt-6 bg-background rounded-md p-4 opacity-50">
                                        <MarkdownViewer content={section.content} urlTransform={(uri) => resolveResourceUrl(filePath, uri)} />
                                    </div>
                                </div>
                            );
                        }
                        ...
                        return isEditing ? (
                            <TiptapWrapper
                                ...
                                editable={true}
                                onSaveAsset={async (file) => await dataManager.saveAsset(entryId, file)}
                                basePath={filePath}
                            />
                        ) : (
                            <MarkdownViewer content={section.content} urlTransform={(uri) => resolveResourceUrl(filePath, uri)} />

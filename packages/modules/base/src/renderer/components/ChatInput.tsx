...
import { InlineEntrySelector } from '@citadel-app/ui';
...
import { CodexEntry, db } from '../lib/db';
...
export const ChatInput = ({
    ...
    const mentionEntries = useLiveQuery(
        async () => {
            if (!mentionQuery) return [];
            return await db.entries.filter(e => e.title.toLowerCase().includes(mentionQuery.toLowerCase())).limit(5).toArray();
        },
        [mentionQuery],
        []
    );

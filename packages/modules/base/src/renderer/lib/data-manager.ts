...
import type { ExternalDataHandler } from '@citadel-app/core';
import { appModuleRegistry } from '../host-services';

// Default external data handlers for types NOT yet owned by a module.
// These are merged with module-registered handlers at runtime.
const DEFAULT_EXTERNAL_HANDLERS: ExternalDataHandler[] = [
    { type: 'highlights', frontmatterKey: 'highlightsId', dir: 'highlights' },
    { type: 'code',       frontmatterKey: 'codeId',       dir: 'code' },
];

/** Returns all external data handlers (defaults + module-registered). */
function getExternalHandlers(): ExternalDataHandler[] {
    const moduleHandlers = appModuleRegistry.getExternalDataHandlers();
    // Merge: module handlers take precedence (by type key)
    const merged = new Map<string, ExternalDataHandler>();
    for (const h of DEFAULT_EXTERNAL_HANDLERS) merged.set(h.type, h);
    for (const h of moduleHandlers)              merged.set(h.type, h);
...
export class DataManager {
    ...
    async init(customPath?: string) {
        ...
        if (customPath) {
            ...
            const context = await __hostApi.app.getInitContext();
        ...
        try {
            await __hostApi.module.invoke('@citadel-app/base', 'db.initWorkspace', this.rootPath);
        } catch (dbInitErr: any) {
        ...
        await __hostApi.module.invoke('@citadel-app/base', 'fs.watchPath', this.rootPath);
        __hostApi.on('fs:file-changed', (event) => this.handleFileChange(event));
    }
    ...
    async createEntry(entry: Partial<CodexEntry> & { title: string, type: CodexEntry['type'] }) {
        ...
        // Separate externalized fields (dynamic, handler-driven)
        const handlers = getExternalHandlers();
        const externalFields: Record<string, any> = {};
        const rest = { ...entry } as any;
        for (const h of handlers) {
            if (rest[h.type] !== undefined) {
                externalFields[h.type] = rest[h.type];
                delete rest[h.type];
            }
        }
        const { frontmatter: incomingFM, ...directFM } = rest as any;
        ...
        for (const h of handlers) {
            delete (frontmatter as any)[h.type];
        }
        ...
        const newEntry: CodexEntry = {
            ...
            updatedAt: now,
            frontmatter,
            highlights: externalFields.highlights || [],
        };
        // Assign each external field to the entry
        for (const h of handlers) {
            if (externalFields[h.type] !== undefined) {
                (newEntry as any)[h.type] = externalFields[h.type];
            }
        }

        // Handle External Storage (generic handler loop)
        for (const h of handlers) {
            const data = externalFields[h.type];
            if (data) {
                const id = await this.writeExternalMetadata(filePath, h.dir, data);
                (frontmatter as any)[h.frontmatterKey] = id;
                (newEntry as any)[h.type] = data;
            }
        }
    ...
    async updateEntry(id: string, updates: Partial<CodexEntry>) {
        ...
        const { frontmatter: updateFrontmatter, ...remainingUpdates } = rest as any;
        ...
        // Clean up externalized fields from frontmatter
        for (const h of handlers) {
            delete updatedEntry.frontmatter[h.type];
        }

        // Update in-memory entry fields (for DB)
        for (const h of handlers) {
            if (externalFields[h.type]) (updatedEntry as any)[h.type] = externalFields[h.type];
        }
        ...
        for (const h of handlers) {
            const data = externalFields[h.type];
            if (data) {
                const id = await this.writeExternalMetadata(entry.filePath, h.dir, data, updatedEntry.frontmatter[h.frontmatterKey]);
                updatedEntry.frontmatter[h.frontmatterKey] = id;
            }
        }
    ...
    async deleteEntry(id: string) {
        ...
        const handlers = getExternalHandlers();
        const externalMappings = handlers.map(h => ({
            id: entry.frontmatter[h.frontmatterKey],
            dir: h.dir
        }));
    ...
    async updateMetadata(id: string, metadata: Partial<any>) {
        ...
        // Merge Metadata
        const updatedFrontmatter = { ...entry.frontmatter, ...rest, updated: new Date().toISOString() };
        
        // Ensure externalized fields are NOT in frontmatter
        for (const h of handlers) {
            delete updatedFrontmatter[h.type];
        }
    ...
    async loadConfig(defaultConfig: WorkspaceConfig): Promise<WorkspaceConfig> {
        ...
        if (await fs.exists(configPath)) {
            try {
                ...
                // but merge them with defaultConfig values AND a generic UI stub for structural completeness
                const BASE_ENTRY_STUB = {
                    icon: 'FileText',
                ...
                if (loaded.entries) {
                    for (const [key, value] of Object.entries(loaded.entries)) {
                        ...
                        mergedConfig.entries[key] = {
                            ...BASE_ENTRY_STUB,
                            // @ts-ignore
                            ...(defaultConfig.entries[key] || {}), // Get defaults if any exist
                            // @ts-ignore
                            ...value
                        };
                    }
    ...
    private async writeExternalMetadata(filePath: string, dir: string, data: any, existingId?: string): Promise<string> {
        const normalizedPath = filePath.replace(/\\/g, '/');
        const entryDir = normalizedPath.substring(0, normalizedPath.lastIndexOf('/'));
        
        const id = existingId || uuidv4();
                        
        const folderPath = `${entryDir}/${dir}`;

...
export const SummaryDialog = ({
    ...
    const generateSummary = async () => {
        try {
            ...
            const { available } = await __hostApi.module.invoke('@citadel-app/base', 'ai.isAvailable');
            if (available && autoIndexEnabled) {
                setStatus('indexing');
                setIndexInfo('Checking index status...');

                const needsIndex = await __hostApi.module.invoke('@citadel-app/base', 'ai.needsIndexing', entry.id, reindexIntervalHours);
                if (needsIndex) {
                    setIndexInfo('Indexing entry for semantic search...');
                    const result = await __hostApi.module.invoke('@citadel-app/base', 'ai.indexEntry', entry, {
                ...
                const structuralContext = await __hostApi.module.invoke('@citadel-app/base', 'ai.getStructuralContext', entry.id, 3);
                const semanticContext = await __hostApi.module.invoke('@citadel-app/base', 'ai.getContext', 
                    entry.id,
...
`;
                ...
                const result = await __hostApi.module.invoke('@citadel-app/base', 'ai.generateSummary', { content: contentToSummarize, context: fullRagContext });
                if (result) {
                    setSummary(result);
                    setStatus('review');
                } else {
                ...
                const contentToSummarize = (!content || content.trim().length === 0)
                    ? `Title: ${entry.title}`
                    : content;
                const result = await __hostApi.module.invoke('@citadel-app/base', 'ai.generateSummary', { content: contentToSummarize });
                if (result) {
                    setSummary(result);
                    setStatus('review');
                } else {
    ...
    const handleApply = async () => {
        ...
        __hostApi.module.invoke('@citadel-app/base', 'ai.indexEntry', entry).catch(e => console.error('[SummaryDialog] Proactive index failed', e));
        onOpenChange(false);
    };

...
export const GrammarDialog = ({
    ...
    const generateProofread = async () => {
        setStatus('loading');
        try {
            const result = await __hostApi.module.invoke('@citadel-app/base', 'ai.proofread', { content });
            if (result) {
    ...
    const handleApply = async () => {
        ...
        __hostApi.module.invoke('@citadel-app/base', 'ai.indexEntry', entry).catch(e => console.error('[GrammarDialog] Proactive index failed', e));
        onOpenChange(false);
    };

...
export const ReplPage = () => {
    ...
    const handleStartSession = async () => {
        ...
        setIsStarting(true);
        try {
            const id = await __hostApi.module.invoke('@citadel-app/code', 'repl:start-session', selectedLang.id);
            setSessionId(id);
        } catch (error) {
    ...
    const handleStopSession = async () => {
        if (!sessionId) return;
        try {
            await __hostApi.module.invoke('@citadel-app/code', 'repl:stop-session', sessionId);
            setSessionId(null);
        } catch (error) {
    ...
    const handleInput = useCallback((data: string) => {
        if (sessionId) {
            __hostApi.module.invoke('@citadel-app/code', 'repl:send-input', sessionId, data);
        }
    }, [sessionId]);

...
export const ReplTerminal = ({ sessionId, onInput }: ReplTerminalProps) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = (window as any).api.repl.onOutput((payload: { sessionId: string, data: string }) => {
            if (payload.sessionId === sessionIdRef.current) {
                term.write(payload.data);
            }
        });

        const unsubscribeClosed = (window as any).api.repl.onClosed((payload: { sessionId: string, code: number }) => {
            if (payload.sessionId === sessionIdRef.current) {

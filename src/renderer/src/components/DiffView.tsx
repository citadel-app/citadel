...
import { MonacoDiffWrapper } from './editors/MonacoDiffWrapper';
...
export const DiffView = ({ file, status, gitStatus, onClose, onDiscard, onSaveSuccess }: DiffViewProps) => {
            ...
            try {
                ...
                if (status === 'index') {
                    ...
                    if (gitStatus === 'A') {
                        ...
                        original = await window.api.git.show(vaultPath, `HEAD:${file}`).catch(() => '');
                    }
                    modified = await window.api.git.show(vaultPath, `:${file}`).catch(() => '');
                } else {
                    ...
                    if (gitStatus === '?') {
                        ...
                        try {
                            original = await window.api.git.show(vaultPath, `:${file}`);
                        } catch {
                            original = '';
                        }
                    }
                    ...
                    if (gitStatus === 'D') {
                        ...
                        try {
                            modified = await window.api.fs.readFile(`${vaultPath}/${file}`);
                        } catch (e) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await window.api.fs.writeFile(fullPath, modifiedContent);
            setIsModified(false);

...
export const FeedView = ({ selectedFeedId, initialItemId }: FeedViewProps) => {
    const navigate = useNavigate();
    const { feeds, itemStatus, markAsRead, markAllAsRead, showUnreadOnly, setShowUnreadOnly, refreshFeed } = useRSS();
    const { splitOrientation, activePanel, setActivePanel } = useLayout();

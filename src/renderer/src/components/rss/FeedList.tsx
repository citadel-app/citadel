...
export const FeedList = ({ selectedFeedId, onSelectFeed, onAddFeed, onToggleSidebar }: FeedListProps) => {
    const { feeds, itemStatus, removeFeed, refreshFeeds, refreshFeed, isLoading, pinnedFolders, togglePinnedFolder } = useRSS();
    ...
    useEffect(() => {
        if (feeds.length > 0 && !isInitialized) {
            const allFolders = new Set(feeds.map(f => f.folder || 'Uncategorized'));

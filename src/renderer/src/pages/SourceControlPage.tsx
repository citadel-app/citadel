...
export const SourceControlPage = () => {
    ...
    const handleClone = async () => {
        ...
        setIsCloning(true);
        try {
            await window.api.git.clone(cloneUrl, target);
    ...
    const handleInit = async () => {
        ...
        try {
            await window.api.git.init(vaultPath);
            await refreshStatus();
        } finally {
    ...
    const handleStage = useCallback(async (file: string) => {
        if (!vaultPath) return;
        try {
            await window.api.git.add(vaultPath, [file]);
            await refreshStatus();
    ...
    const handleUnstage = useCallback(async (file: string) => {
        if (!vaultPath) return;
        try {
            await window.api.git.unstage(vaultPath, [file]);
            await refreshStatus();
    ...
    const confirmDiscard = async () => {
        ...
        try {
            await window.api.git.discard(vaultPath, discardDialog.file);
            await refreshStatus();
    ...
    const handleUnstageAll = useCallback(async () => {
        ...
        try {
            ...
            await window.api.git.unstage(vaultPath, stagedFiles.map(f => f.path));
            await refreshStatus();
        } finally {
            setActionLoading(false);
        }
    ...
    const confirmDiscardAll = async () => {
        ...
        setActionLoading(true);
        try {
            await window.api.git.discardBulk(vaultPath, changesFiles.map(f => f.path));
            await refreshStatus();
    ...
    const handleCommit = async () => {
        ...
        setActionLoading(true);
        try {
            await window.api.git.commit(vaultPath, commitMessage);
            setCommitMessage('');
            await refreshStatus();
        } finally {
            setActionLoading(false);
    ...
    const handleAddRemote = async () => {
        ...
        try {
            await window.api.git.addRemote(vaultPath, remoteName, remoteUrl);
        ...
        try {
            ...
            if (isHandledError) {
                ...
                if (!remotes || remotes.length === 0) {
                    ...
                    try {
                        await window.api.git.push(vaultPath, remoteName, status?.current || defaultBranch);

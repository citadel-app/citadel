...
import { DynamicIcon as Icon } from '../components/IconRegistry';
...
export const ReplPage = () => {
    ...
    const handleStartSession = async () => {
        ...
        setIsStarting(true);
        try {
            const id = await (window as any).api.repl.startSession(selectedLang.id);
            setSessionId(id);
        } catch (error) {
    ...
    const handleStopSession = async () => {
        if (!sessionId) return;
        try {
            await (window as any).api.repl.stopSession(sessionId);
            setSessionId(null);
        } catch (error) {
    ...
    const handleInput = useCallback((data: string) => {
        if (sessionId) {
            (window as any).api.repl.sendInput(sessionId, data);
        }
    }, [sessionId]);
                        ...
                        <>
                            {languages.map((lang) => (
                                <button
                                    ...
                                    title={lang.name}
                                >
                                    <Icon name={lang.icon as any} size={18} className={cn(selectedLang.id === lang.id ? lang.color : "opacity-40 group-hover:opacity-100")} />
                    ...
                    <div className="absolute inset-0 z-10 flex items-center justify-center">
                        <div className="max-w-md text-center space-y-6">
                            <div className="w-24 h-24 rounded-[2.5rem] bg-muted/20 flex items-center justify-center mx-auto mb-8 border border-white/5 shadow-2xl">
                                <Icon name={selectedLang.icon as any} size={48} className={cn(selectedLang.color, "opacity-20")} />
                            </div>

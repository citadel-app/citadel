...
import { FileTree } from './LatexEditor/FileTree';
...
export const LatexEditorPage = () => {
    ...
    useEffect(() => {
        if (window.api && window.api.latex) {
            window.api.latex.check().then(available => {
    ...
    const handleManualCompile = async () => {
        ...
        if (isProMode) {
            ...
            try {
                const result = await window.api.latex.compile(files);
    ...
    const handleDeleteFile = (path: string) => {
        ...
        if (files.length <= 1 && files[0].name === path) return; // Prevent deleting last file if it's the only one? Actually maybe allow if we create default?
        // Let's keep existing logic: prevent deleting last file
    ...
    const handleUploadFile = async () => {
        try {
            ...
            if (isImage) {
                const buffer = await window.api.fs.readFileBinary(filePath);
                if (buffer) {
                    // Convert to base64
                    let binary = '';

...
import { getPrimaryFieldForUrl } from '../lib/module-utils';
import * as DropdownMenu from '@radix-ui/react-dropdown-menu';
import { useMemo, useState, useRef, useEffect } from 'react';
import { useWebviewAudio } from '../hooks/useWebviewAudio';
import { useYouTubePlayer } from '@renderer/context/YouTubePlayerContext';
...
const ConvertMenu = ({ item }: { item: FeedItem }) => {
    ...
    const handleConvert = async (typeKey: string) => {
        ...
        const urlField = getPrimaryFieldForUrl(entryType, config.modules || {});
        ...
        try {
            const entry = await dataManager.createEntry(entryData);
        ...
        <DropdownMenu.Root>
            ...
            <DropdownMenu.Portal>
                <DropdownMenu.Content
                    ...
                    sideOffset={5}
                >
                    <DropdownMenu.Label className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                        Convert to...
                    </DropdownMenu.Label>
...
export const YouTubePage = () => {
    ...
    const handleReset = async (id: string) => {
        if (confirm('Are you sure you want to reset this feed? All existing videos will be cleared and re-fetched.')) {
            await resetFeed(id);
        }
    };

import { EntryTypeConfig, ModuleDefinition } from '../config/entry-types';
...
export const getPrimaryFieldForUrl = (
    ...
    // BUT strictly, if shorthand, no map exists. 
    // So we usually rely on 'source' -> 'source' implicit mapping.
    
    // If it's an object config, we check the map.
    ...
    // This handles cases like 'source' -> 'source' without explicit map.
    ...
    // If still nothing, but the requirement is exactly 'source' or 'url', 
    // and we have a 'sourceUrl' or 'pdfPath' etc... 
    // This gets fuzzy. Sticking to strict mapping logic means we might return null here.
    // But the user requested "same behavior we found in module mapping".
    // In GenericEntryView, we often just pass the data if the keys match.

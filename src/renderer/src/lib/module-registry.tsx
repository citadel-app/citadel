...
function createScopedApi(moduleId: string, allowedMethods: string[]): ScopedAPI {
    ...
    for (const method of allowedMethods) {
        ...
        if (!ns || !fn) {
            console.warn(`[ModuleRegistry] Invalid permission format: "${method}". Expected "namespace.method".`);
            continue;
        }
...
export class ModuleRegistry implements RendererRegistrar {
    ...
    private globalProviders: { entry: ProviderRegistration; component: React.ComponentType<any>; moduleId: string }[] = [];
    ...
    async loadModules(modules: IModule[]) {
        for (const module of modules) {
            ...
            // Static validation
            if (perms.length === 0) {
                console.log(`[ModuleRegistry] Activating module: ${module.id} (v${module.version}) — no IPC permissions`);
            } else {
                console.log(`[ModuleRegistry] Activating module: ${module.id} (v${module.version}) — IPC: [${perms.join(', ')}]`);
            }
        ...
        // Process declarative registrations
            if (module.contentModules) {
                for (const [key, cm] of Object.entries(module.contentModules)) this.contentModules.set(key, cm);
            }
            if (module.contentViewers) {
                for (const [et, comp] of Object.entries(module.contentViewers)) this.registerContentViewer(et, comp);
            }
            ...
            if (module.globalComponents) {
                for (const gc of module.globalComponents) this.registerGlobalComponent(gc.region, gc.component);
            }
            if (module.routes) {

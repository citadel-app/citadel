import { AIModel, OllamaGenerateResponse, AIModelSettings } from '../types';
...
export class OllamaClient {
    ...
    async abortChat() {
        return window.api.ai.abortChat();
    }
    ...
    async getModels(): Promise<AIModel[]> {
        try {
            ...
            console.error('[OllamaClient] Failed to get models:', e);
            throw e;
        }
    }
    ...
    async generate(options: {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`[OllamaClient] Generation failed (${response.status}):`, errorText);
                return null;
            }

            const data: OllamaGenerateResponse = JSON.parse(response.text);
            return data.response || null;
        } catch (e) {
            console.error('[OllamaClient] Request failed:', e);
            return null;
        }
    }
    ...
    async embed(text: string, model: string = 'nomic-embed-text'): Promise<number[] | null> {
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error(`[OllamaClient] Embedding failed (${response.status}):`, errorText);
                return null;
            }
            const data = JSON.parse(response.text);
            return data.embeddings?.[0] || null;
        } catch (e) {
            console.error('[OllamaClient] Embedding failed:', e);
            return null;
        }
    }
    ...
    async getSettings(): Promise<AIModelSettings> {
        ...
        try {
            ...
            console.warn("[OllamaClient] Failed to fetch settings:", e);
        }
    ...
    async pullModel(model: string, onProgress?: (data: { status: string; completed?: number; total?: number; digest?: string }) => void): Promise<boolean> {
        ...
        try {
            ...
            // Use main process to pull via IPC
            await window.api.ai.pullModel(this.baseUrl, model);
            return true;
        } catch (e) {
            console.error('[OllamaClient] Failed to pull model:', e);
    ...
    scoreModel(model: import('../types').PopularModel, specs: { totalMemory: number, gpus: { model: string, vram: number }[] }): { score: 'excellent' | 'good' | 'poor', reason: string } {

...
export const RSSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setIsLoading(true);
            try {
                const savedData = (await dataManager.loadFeeds()) as any;
                const savedStatus = await (window as any).api.db.getFeedStatus();
                ...
                if (savedData !== null) {
                    ...
                    for (const feed of feedsToSet) {
                        try {
                            const cachedItems = await (window as any).api.db.getFeedItems(feed.id, 200);
                            ...
                            if (feed.items && feed.items.length > 0 && cachedItems.length === 0) {
                                ...
                                await (window as any).api.db.saveFeedItems(feed.id, feed.items);
                            }
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            // Items are stripped in dataManager.saveFeeds, so we just pass feeds
            dataManager.saveFeeds({ feeds: f, pinnedFolders: p } as any).catch(() => { });
    ...
    const addFeed = useCallback(async (url: string, folder?: string, title?: string) => {
        ...
        try {
            ...
            if (newFeed.items.length > 0) {
                await (window as any).api.db.saveFeedItems(newFeed.id, newFeed.items);
            }
    ...
    const removeFeed = useCallback(async (id: string) => {
        ...
        if (feed) {
            ...
            if (itemIds.length > 0) {
                await dataManager.removeRelatedLinks(itemIds, 'rss-item');
            }
        }
    ...
    const refreshFeed = useCallback(async (id: string, isAuto = false) => {
        ...
        try {
            ...
            if (data.items && data.items.length > 0) {
                await (window as any).api.db.saveFeedItems(feed.id, data.items);
            }
        ...
        if (updates.url) {
            ...
            if (feed && feed.url !== updates.url) {
                try {
                    ...
                    if (data.items && data.items.length > 0) {
                        await (window as any).api.db.saveFeedItems(id, data.items);
                    }
    ...
    const markAsRead = useCallback((_feedId: string, itemId: string) => {
        setItemStatus(prev => {
            ...
            (window as any).api.db.updateFeedStatus(itemId, nextStatus[itemId]).catch((e: any) => console.error(e));
            return nextStatus;
        });
    }, []);
    ...
    const markAllAsRead = useCallback((feedId?: string) => {
        setItemStatus(prev => {
            ...
            itemsToMark.forEach(item => {
                ...
                (window as any).api.db.updateFeedStatus(item.id, next[item.id]).catch((e: any) => console.error(e));
            });
            return next;
        });
    }, []);
    ...
    const addYouTubeChannel = async (_input: string) => {
        // Obsolete: YouTube logic moved to YouTubeContext
    };

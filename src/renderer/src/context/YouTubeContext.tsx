...
export const YouTubeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setIsLoading(true);
            try {
                const savedFeeds = await dataManager.loadYouTubeFeeds();
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            // Items are stripped in dataManager.saveYouTubeFeeds, so we just pass feeds
            dataManager.saveYouTubeFeeds(f);
        ...
        try {
            ...
            try {
                ...
                if (feedData.items && feedData.items.length > 0) {
                    await (window as any).api.db.saveFeedItems(newFeedId, feedData.items);
                }
    ...
    const updateChannel = useCallback(async (id: string, input: string) => {
        ...
        try {
            ...
            try {
                ...
                if (feedData.items && feedData.items.length > 0) {
                    await (window as any).api.db.saveFeedItems(id, feedData.items);
                }
    ...
    const refreshFeed = useCallback(async (id: string) => {
        ...
        try {
            ...
            if (feedData.items && feedData.items.length > 0) {
                await (window as any).api.db.saveFeedItems(feed.id, feedData.items);
            }
